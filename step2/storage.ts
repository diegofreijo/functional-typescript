...
export type RowInserter = (sql: string, parameters: { $user: string, $pass: string }) => Promise<boolean>;
...
export async function createUser(inserter: RowInserter, user: string, pass: string): Promise<boolean> {
	return inserter(
